...
class feeModel extends CI_Model 
{
  ...
  public function harvest(){
      ...
      $query = $this->db->query("SELECT name,priority,building,class,description,assigned_to,status,date_posted FROM fee WHERE status = 'Pending'");

      if($query->num_rows()){
          foreach ($query->result() as $row)
         {
            echo '<hr>' . $row->name .' fee assigned to '. $row->assigned_to  .'<br>';
         }
