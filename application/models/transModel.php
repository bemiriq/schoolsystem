...
class transModel extends CI_Model 
{
  ...
  function get_fichas() {
    $this->db->select('*')->from('transportation');
    $query=$this->db->get();
  ...
  public function harvest(){
      ...
      $query = $this->db->query("SELECT name,priority,building,class,description,assigned_to,status,date_posted FROM transportation WHERE status = 'Pending'");

      if($query->num_rows()){
          foreach ($query->result() as $row)
         {
            echo '<hr>' . $row->name .' transportation assigned to '. $row->assigned_to  .'<br>';
         }
